...
const tipoColor = {
  'autoatendible':  'bg-green-100 text-green-800',
  'necesita-ayuda': 'bg-blue-100 text-blue-800',
  'solo-911':       'bg-red-100 text-red-800',
}

const tipoLabel = {
  'autoatendible':  'Tú puedes',
  'necesita-ayuda': 'Con ayuda',
  'solo-911':       'Solo 911',
}
...
export default function AdminDashboard() {
  ...
  async function toggleActivo(id, valorActual) {
    await supabase.from('emergencias').update({ activo: !valorActual }).eq('id', id)
    setEmergencias(prev => prev.map(e => e.id === id ? { ...e, activo: !valorActual } : e))
  }
    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          <button
            onClick={() => navigate('/admin/emergencias/nueva')}
        ...
        {cargando ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 text-sm">Cargando emergencias...</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-800">
                  <th className="text-left text-xs font-medium text-gray-400 px-4 py-3 uppercase tracking-wider">Emergencia</th>
                  <th className="text-left text-xs font-medium text-gray-400 px-4 py-3 uppercase tracking-wider hidden sm:table-cell">Urgencia</th>
                  <th className="text-left text-xs font-medium text-gray-400 px-4 py-3 uppercase tracking-wider hidden md:table-cell">Tipo</th>
                  <th className="text-left text-xs font-medium text-gray-400 px-4 py-3 uppercase tracking-wider hidden md:table-cell">Sección</th>
                  <th className="text-left text-xs font-medium text-gray-400 px-4 py-3 uppercase tracking-wider">Estado</th>
                  <th className="text-right text-xs font-medium text-gray-400 px-4 py-3 uppercase tracking-wider">Acciones</th>
                </tr>
              </thead>
                ...
                {filtradas.map(e => (
                  <tr key={e.id} className="hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="px-4 py-3 hidden sm:table-cell">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${urgenciaColor[e.urgencia]}`}>
                        {e.urgencia}
                      </span>
                    </td>
                    <td className="px-4 py-3 hidden md:table-cell">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${tipoColor[e.tipo_protocolo]}`}>
                        {tipoLabel[e.tipo_protocolo]}
                      </span>
                    </td>
                    <td className="px-4 py-3 hidden md:table-cell">
                    ...
                    <td className="px-4 py-3">
                      <button
                        ...
                        className={`relative w-10 h-5 rounded-full transition-colors ${
                          e.activo ? 'bg-green-600' : 'bg-gray-600'
                        }`}
                      >
                    ...
                    <td className="px-4 py-3 text-right">
                      <button
                        onClick={() => navigate(`/admin/emergencias/${e.id}`)}
                        className="text-xs px-3 py-1.5 rounded-lg border border-gray-700
